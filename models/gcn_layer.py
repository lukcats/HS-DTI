...
class GraphConvolution(Module):
    ...
    def norm(self, adj):  # D(-1/2)AD(-1/2)
        ...
        norm_adj = torch.matmul(torch.matmul(diag_deg, adj_post), diag_deg)  # D(-1/2)AD(-1/2)
        return norm_adj

    def forward(self, input, adj):
        support = torch.matmul(input, self.weight)  # A'XW
        adj_norm = self.norm(adj)
        output = torch.matmul(support.transpose(1, 2), adj_norm.transpose(1, 2))  # transpose(1, 2)
